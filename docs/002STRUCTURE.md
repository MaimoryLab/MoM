...
│       │   ├── ChatPage.tsx       # 更新 — ISS-037；传统 chatbot 布局：header（title + history 下拉，无 subtitle）+ 消息区（空态时预设卡片居中显示，有 comparison 时 iMessage 气泡上下滚）+ sticky 底部 composer（textarea + Submit，Enter 发送 / Shift+Enter 换行）；MoM 回复走 MarkdownBody flush 模式完全展开无内滚；后端仍并发跑 baseline+judge（数据到 Live 页）
