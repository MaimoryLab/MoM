...
│       │   ├── PipelinePage.tsx   # 更新 — ISS-052 / ISS-056 / ISS-057 / ISS-058；AdvisorCard / AggregatorCard 接入 useTypewriter，kiosk.enabled && status==='done' 时 preview 打字机 + scrollRef 自动滚到底；turn.nodes.length===0 时显示提示卡片而非光秃箭头；ISS-056 起 TurnSelect 文案与 Live 页 RunSelect 一致（time · clipped-prompt）；ISS-058 定型：aggregator traces 为主源担保"能画流程"，comparisons 作 prompt 增补，缺 prompt 时 fallback 到 hash8 + model
