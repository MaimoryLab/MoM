...
│       │   ├── PipelinePage.tsx   # 更新 — ISS-052 / ISS-056 / ISS-057；AdvisorCard / AggregatorCard 接入 useTypewriter，kiosk.enabled && status==='done' 时 preview 打字机 + scrollRef 自动滚到底；turn.nodes.length===0 时显示提示卡片而非光秃箭头；ISS-056 起 TurnSelect 文案与 Live 页 RunSelect 一致（time · clipped-prompt）；ISS-057 起数据源改成 listComparisons(20) ∩ listTraces(role=aggregator) 交集，保证每条 option 都能画流程图
