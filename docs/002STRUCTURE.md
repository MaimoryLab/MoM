...
│       │   ├── PipelinePage.tsx   # 更新 — ISS-052 / ISS-056 / ISS-059；AdvisorCard / AggregatorCard 接入 useTypewriter，kiosk.enabled && status==='done' 时 preview 打字机 + scrollRef 自动滚到底；turn.nodes.length===0 时显示提示卡片而非光秃箭头；ISS-059 定型：dropdown 数据源与 Live RunSelect 完全同源（`listComparisons(20)`），文案统一 `time · clipped-prompt`；无 aggregator trace 的 gwId 点开后由现有空态卡兜底（ISS-057 / ISS-058 的严格交集 + fallback 方案被 ISS-059 撤销）
