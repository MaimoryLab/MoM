...
- **MoM usage/cost 汇总口径**（ISS-063 起）：`comparisons.mom_usage_json` / `mom_cost_usd` 存的是本轮所有 role∈{advisor, aggregator} trace 的累加值，不是 aggregator 单次调用的 usage。前端 StatsRow 显示 `input+cache_read+cache_creation+output` 全部四维之和，与 `mom_cost_usd` 同源，可反向校对
- **启动自愈：僵尸 pending 兜底**（ISS-064 起）：`initDB` 之后立即 `markStalePendingComparisonsAsError()`，把所有 `status='pending'` 行改成 `error` + `mom_error_json='server restarted while running'`。杀进程 / 崩溃遗留的 pending 行不会永远卡在前端 3s 轮询里；用户可再走 DELETE 清掉
